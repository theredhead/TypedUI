...
(function (red) {
    ...
    function resizeProportionally(r, oldContainer, newContainer) {
        //console.log('resizeProportionally', arguments);
        var factor = 100, factorX = ((factor / oldContainer.size.width) * (newContainer.size.width)) / factor, factorY = ((factor / oldContainer.size.height) * (newContainer.size.height)) / factor, origin = PointMake(r.origin.x * factorX, r.origin.y * factorY), size = SizeMake(r.size.width * factorX, r.size.height * factorY);
            ...
            if (oldSize != null) {
                if (this.autoresizesSubviews) {
                    ...
                    for (var ix = 0; ix < this._subViews.length; ix++) {
                        var sub = this._subViews[ix], rect = resizeProportionally(sub.frame, oldRect, this.frame);
    ...
    var Window = (function (_super) {
        ...
        function Window(aRect) {
            ...
            var tw = aRect.size.width / 5, th = aRect.size.height / 5, testView = new View(RectMake(tw, th, tw, th));
            testView.setBackgroundColor(colors.green);
            testView.autoresizingMask = 2 /* LockedLeft */ | 8 /* LockedRight */ | 4 /* LockedBottom */;
